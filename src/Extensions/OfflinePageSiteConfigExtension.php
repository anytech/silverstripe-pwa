...
class OfflinePageSiteConfigExtension extends Extension
{
    ...
    {
        // Content
        $fields->addFieldToTab('Root.PWA.OfflinePage', HeaderField::create('OfflineContentHeader', 'Offline Page Content'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', LiteralField::create('OfflineInfo',
            '<p class="message info">Customize the page shown when users are offline and try to navigate to a page that isn\'t cached.</p>'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', TextField::create('OfflineTitle', 'Title')
            ->setDescription('Main heading shown on the offline page'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', TextareaField::create('OfflineMessage', 'Message')
            ->setRows(3)
            ->setDescription('Explanation text shown below the title'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', TextField::create('OfflineButtonText', 'Button Text')
            ->setDescription('Text for the retry button'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', \SilverStripe\Forms\DropdownField::create('OfflineIcon', 'Icon', self::$offline_icons)
            ->setDescription('Emoji icon shown on the offline page'));

        // Styling
        $fields->addFieldToTab('Root.PWA.OfflinePage', HeaderField::create('OfflineStyleHeader', 'Offline Page Styling'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', TextField::create('OfflineBackgroundColor', 'Background Color')
            ->setAttribute('type', 'color')
            ->setDescription('Background color of the offline page'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', TextField::create('OfflineTextColor', 'Text Color')
            ->setAttribute('type', 'color')
            ->setDescription('Color of the text on the offline page'));

        $fields->addFieldToTab('Root.PWA.OfflinePage', TextField::create('OfflineAccentColor', 'Accent Color')
            ->setAttribute('type', 'color')
            ->setDescription('Color for the title and button'));

        // Preview
        $fields->addFieldToTab('Root.PWA.OfflinePage', HeaderField::create('OfflinePreviewHeader', 'Preview'));
        ...
        $fields->addFieldToTab('Root.PWA.OfflinePage', LiteralField::create('OfflinePreview',
